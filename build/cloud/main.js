...
Parse.Cloud.define("posts", async ({ request }) => {
    ...
    if ((request.params.user == undefined || request.params.user == '') && request.params.project == undefined) {
        Posts.containedIn("tags", request.params.tags);
    } //for loading posts related to certain tags
    else if (request.params.project)
        Posts.equalTo('referredTo', request.params.project); //for loading posts related to a project
    else if (request.params.user != request.params.me)
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        let liked = '';
        let quoted = '';
        if (likes.indexOf(me.get('username')) > -1) {
            liked = 'lq';
        }
        if (quotes.indexOf(me.get('username')) > -1) {
            quoted = 'lq';
        }
...
Parse.Cloud.define("projects", async ({ request }) => {
    ...
    if (request.params.user == undefined || request.params.user == '') {
        Projects.containedIn("tags", request.params.tags);
        Projects.limit(10);
    }
    else if (request.params.user != request.params.me) {
        Projects.equalTo('user', request.params.user);
    }
    ;
    if (request.params.sortby == 'oldest')
        Projects.descending('createdAt');
    if (request.params.sortby == 'latest')
        Projects.ascending('createdAt');
    else
        Projects.descending('createdAt');
...
Parse.Cloud.define("like", async (request) => {
    ...
    if (request.params.wh == 'add') {
        post.addUnique('likes', request.params.username);
    }
    else {
        post.remove('likes', request.params.username);
    }
    await post.save(null, { useMasterKey: true });
    return 'done';
});
Parse.Cloud.define("quote", async (request) => {
    let post = new Parse.Object("Posts");
    post.id = request.params.postId;
    if (request.params.wh == 'add') {
        post.addUnique('quotes', request.params.username);
    }
    else {
        post.remove('quotes', request.params.username);
    }
    await post.save(null, { useMasterKey: true });
    return 'done';
});
Parse.Cloud.define("like_comment", async (request) => {
